...
\begin{itemize}
    ...
    \item \textbf{Derived \texttt{UI::Canvas} Classes (e.g., \texttt{UI::AVL\_Canvas}, \texttt{UI::MST\_Canvas}):} Inherit from \texttt{UI::\allowbreak Canvas} to render specific structures. To maintain strict encapsulation, each derived class independently manages its own UI playback buttons (\texttt{next()}, \texttt{prev()}, \texttt{skip()}) and playback state variables (\texttt{current\_step}, \texttt{is\_playing}).
